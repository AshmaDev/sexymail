...
const DEFAULT_BORDER = "#e4e2e2";

const DEFAULT_RADIUS: Required<Radius> = {
  card: 15,
  control: 10,
  image: 5,
};
...
class SexyMail {
  ...
  private static containerStyles =
    "background-color:{background}; height:auto !important;width:100% !important; font-family: Helvetica,Arial,sans-serif !important; padding-top: 40px; margin-bottom: 40px;";
  private static tableStyles =
  ...
  private static textStyles =
    "color:{text}; font-size: 14px; padding-bottom: 10px; line-height: 1.4;";
  private static linkTableStyles = "min-width:100%;border-collapse:collapse;";
  private static linkStyles =
    "font-size: 14px; line-height: 1.5; font-weight: 700; letter-spacing: 1px; padding: 15px 40px; text-align:center; text-decoration:none; color:{primaryText}; border-radius: {controlRadius}px; background-color:{primary};";
  private static codeBlockStyles =
    "background-color:{background}; color: {headingText}; padding: 8px 15px; border-radius: {controlRadius}px; display: inline-block; margin-bottom:20px; font-size: 24px; line-height: 1.4; font-family: Courier New, Courier, monospace; font-weight: 600; margin-top:0";
  private static listItemStyles = "font-size: 12px; line-height: 1; color:{text};";
  private static priceStyles =
    "font-size: 14px; line-height: 1; color: {primary}; margin-top:5px; vertical-align:top; white-space:nowrap;";
  private static summaryItemStyles =
  ...
  private getRadius(): Required<Radius> {
    return {
      card: this.config.radius?.card ?? DEFAULT_RADIUS.card,
      control: this.config.radius?.control ?? DEFAULT_RADIUS.control,
      image: this.config.radius?.image ?? DEFAULT_RADIUS.image,
    };
  }

  private getBorderColor() {
    return this.config.colors.border?.trim() || DEFAULT_BORDER;
  }
  ...
  private createContainer(content: string, info?: string): string {
    ...
    const tableStyles = SexyMail.tableStyles
      .replace("{foreground}", this.config.colors.foreground)
      .replace("{border}", border)
      .replace("{cardRadius}", String(radii.card));
    const footerStyles = SexyMail.footerStyles.replace(
      "{text}",
        ...
        <center>
          <table border="0" cellpadding="0" cellspacing="0" style="${tableStyles}">
            <tbody>
              ${
                this.config.logo
                  ? `<tr><td align="left" valign="center" style="padding-bottom:40px;border-top:0;height:100% !important;width:100% !important;">${logo}</td></tr>`
                  : ""
              }
              <tr><td style="padding-top:10px;border-top:1px solid ${border}">${content}</td></tr>
            </tbody>
          </table>
          ${
            info
  ...
  private createHeader(title: string): string {
    const headingStyles = SexyMail.headingStyles.replace(
      "{headingText}",
  ...
  private createText(text: string): string {
    const textStyles = SexyMail.textStyles.replace(
      "{text}",
      this.config.colors.text
    );
  ...
  private createLink({ text, url }: Link): string {
    ...
    const linkStyles = SexyMail.linkStyles
      ...
      .replace("{controlRadius}", String(radii.control));
  ...
  private createListItem(item: ListItem): string {
    const radii = this.getRadius();
    const listItemStyles = SexyMail.listItemStyles.replace(
      "{text}",
      this.config.colors.text
    );
    const priceStyles = SexyMail.priceStyles.replace(
      "{primary}",
    ...
    return `
      <tr>
        <td style="padding-top:0px; padding-bottom:20px; width:140px">
          <img src="${item?.img}" style="width: 114px; height: 85px; object-fit: cover; border-radius: ${radii.image}px;" />
        </td>
  ...
  private createSummaryItem(item: SummaryItem): string {
    const summaryItemStyles = SexyMail.summaryItemStyles.replace(
      "{text}",
      this.config.colors.text
    );
    const summaryValueStyles = SexyMail.summaryValueStyles.replace(
      "{text}",
      this.config.colors.text
    );
